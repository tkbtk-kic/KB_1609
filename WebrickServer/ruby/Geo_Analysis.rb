...
# なんか avarageがちゃんと動いてないかも！？
# 違うわ
# neightborhood = 0.0005 約60メートル以内のものをお隣さんとして認識している（自分自身をを含む）
# number=1 お隣さんが一番たくさん持っている人だけ ホットスポットとして認定される
# お隣さんたちのavarageがホットスポットの中心点となる
#
# つまり
# お隣さんたちがある端っこに固まっているとhotspot中心が かなりずれる
# →反対方向の端のほうが 半径約60メートルの円に収まらない用になる

# どう解決するか
# ・いまホットスポットの大きさを計算してないので いい感じに計算する
#   送って表示に反映してもらう
# →だるい
#
# ・ホットスポットを最大120にしてもらう
...
class Geo_Analysis
  ...
  def hot_spots_googlemap
    result={}
    result.store("hot_spot",{id:@hot_spots[0][:id], coordinates:{lat:@hot_spots[0][:coordinates][0],lng:@hot_spots[0][:coordinates][1]}})
    result
  end
end
...
p geo_analysis.hot_spots_googlemap